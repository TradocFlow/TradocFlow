...
/// Translation unit for storing source/target text pairs
#[derive(Debug, Clone, Serialize, Deserialize)]
...
impl TranslationUnit {
    ...
    pub fn new(
        ...
        // Validate languages
        if source_language.trim().is_empty() {
            return Err(ValidationError::InvalidTranslationUnit("Source language cannot be empty".to_string()));
        }
        
        if target_language.trim().is_empty() {
            return Err(ValidationError::InvalidTranslationUnit("Target language cannot be empty".to_string()));
        }

        // Validate text content
        if source_text.trim().is_empty() {
            return Err(ValidationError::InvalidTranslationUnit("Source text cannot be empty".to_string()));
        }

        if target_text.trim().is_empty() {
            return Err(ValidationError::InvalidTranslationUnit("Target text cannot be empty".to_string()));
        }

        // Validate confidence score
        if !(0.0..=1.0).contains(&confidence_score) {
            return Err(ValidationError::InvalidTranslationUnit("Confidence score must be between 0.0 and 1.0".to_string()));
        }

        // Ensure source and target languages are different
        if source_language == target_language {
            return Err(ValidationError::InvalidTranslationUnit("Source and target languages must be different".to_string()));
        }
    ...
    pub fn update_translation(&mut self, target_text: String, confidence_score: f32) -> Result<(), ValidationError> {
        if target_text.trim().is_empty() {
            return Err(ValidationError::InvalidTranslationUnit("Target text cannot be empty".to_string()));
        }
...
impl Term {
    ...
    pub fn new(
        ...
        if term.trim().is_empty() {
            return Err(ValidationError::InvalidTerm("Term cannot be empty".to_string()));
        }

        // Validate definition if provided
        if let Some(ref def) = definition {
            if def.trim().is_empty() {
                return Err(ValidationError::InvalidTerm("Definition cannot be empty if provided".to_string()));
            }
        }
    ...
    pub fn update_definition(&mut self, definition: Option<String>) -> Result<(), ValidationError> {
        if let Some(ref def) = definition {
            if def.trim().is_empty() {
                return Err(ValidationError::InvalidTerm("Definition cannot be empty if provided".to_string()));
            }
...
/// CSV record for terminology import/export
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct TerminologyCsvRecord {
    pub term: String,
    pub definition: Option<String>,
    pub do_not_translate: Option<String>, // "true"/"false" or "yes"/"no"
...
impl TerminologyCsvRecord {
    ...
    pub fn to_term(&self) -> Result<Term, ValidationError> {
...
/// Terminology import result with validation details
#[derive(Debug, Clone)]
...
/// Terminology conflict resolution strategies
#[derive(Debug, Clone, Serialize, Deserialize, PartialEq)]
pub enum ConflictResolution {
    Skip,           // Skip the conflicting term
    Overwrite,      // Overwrite the existing term
    Merge,          // Merge definitions and notes
    CreateVariant,  // Create a variant with suffix
}
...
impl TerminologyValidationConfig {
    ...
    pub fn validate_term(&self, term: &Term) -> Result<(), ValidationError> {
        // Check term length
            ...
            match field.as_str() {
                "term" => {
                    if term.term.trim().is_empty() {
                        return Err(ValidationError::InvalidTerm("Term is required".to_string()));
                    }
                }
                "definition" => {
                    if term.definition.is_none() || term.definition.as_ref().unwrap().trim().is_empty() {
                        return Err(ValidationError::InvalidTerm("Definition is required".to_string()));
                    }
                }
